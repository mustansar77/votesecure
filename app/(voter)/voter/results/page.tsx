  ...
  return (
    <div className="p-6 lg:p-8">
      <div className="mb-8">
        ...
        <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">Final results of closed and announced elections.</p>
      </div>

      {!elections?.length ? (
        <Card className="flex flex-col items-center py-16 text-center">
          <BarChart2 size={48} className="mb-4 text-slate-300 dark:text-slate-600" />
          <p className="text-slate-500 dark:text-slate-400">No completed elections yet.</p>
        </Card>
      ) : (
        <div className="space-y-10">
          {elections.map((election) => {
            ...
            const winnerCandidate = election.winner_id
              ? candidates.find((c) => c.profile_id === election.winner_id)
              : null;
            ...
            return (
              <div key={election.id}>
                {/* Winner banner — only for officially announced elections */}
                {isAnnounced && winnerProfile && (
                  <div className="mb-4 flex flex-wrap items-center gap-4 rounded-xl border-2 border-amber-300 dark:border-amber-700 bg-amber-50 dark:bg-amber-900/20 px-5 py-4">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-xs font-semibold uppercase tracking-wider text-amber-600 dark:text-amber-400">
                        Official Winner — {election.title}
                      </p>
                ...
                <Card className={`p-0 overflow-hidden ${isAnnounced ? "ring-2 ring-amber-300 dark:ring-amber-700" : ""}`}>
                  <div className="px-6 py-4 border-b border-slate-200 dark:border-slate-700 flex flex-wrap items-center justify-between gap-3">
                    <div>
                      ...
                      <p className="text-xs text-slate-400 mt-0.5">{election.position} · {election.location} · {total} total votes</p>
                    </div>
                    ...
                    <div className="overflow-x-auto">
                      <table className="w-full text-sm">
                        <thead className="bg-slate-50 dark:bg-slate-900">
                          <tr>
                            {["#", "Name", "Father Name", "CNIC", "Location", "Votes", "Win %"].map((h) => (
                        ...
                        <tbody className="divide-y divide-slate-100 dark:divide-slate-700">
                          {candidates.map((c, i) => {
                            ...
                            const isWinner = election.winner_id === c.profile_id;
                            return (
                              <tr key={c.id} className={`${isWinner && isAnnounced ? "bg-amber-50 dark:bg-amber-900/10" : i === 0 && !isAnnounced ? "bg-slate-50 dark:bg-slate-800/50" : "hover:bg-slate-50 dark:hover:bg-slate-700/50"} transition-colors`}>
                                <td className="px-4 py-3">
                                  <span className={`inline-flex h-7 w-7 items-center justify-center rounded-full text-xs font-bold
                                    ${isWinner && isAnnounced ? "bg-amber-400 text-white" : i === 1 ? "bg-slate-300 text-slate-700 dark:bg-slate-600 dark:text-white" : i === 2 ? "bg-orange-300 text-white" : "bg-slate-100 text-slate-500 dark:bg-slate-700 dark:text-slate-400"}`}>
                                    {i + 1}
                                  </span>
                                </td>
                                <td className="px-4 py-3 font-semibold text-slate-900 dark:text-white">
                                  {c.profiles?.name ?? "—"}
                                  {isWinner && isAnnounced && <Trophy size={13} className="inline ml-1.5 text-amber-500" />}
                                </td>
